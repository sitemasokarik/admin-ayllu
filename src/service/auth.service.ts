...
export class AuthService {
  ...
  isAdministradorGeneral(): boolean {
    const user = this.getUser();
    if (!user?.rolNombre) return false;

    const normalized = String(user.rolNombre)
      .toLowerCase()
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
      .trim();
    ...
    return (
      ...
      normalized.includes('administrador general')
    );
  }
